...
public partial class GameRunner(GameContext ctx)
{
    ...
    private string? RunGameLoop()
    {
        ...
        while (!Raylib.WindowShouldClose())
        {
            ...
            // Process active work (non-blocking work simulation)
            if (ctx.ActiveWork != null)
            {
                if (ctx.ActiveWork.AwaitingContinueDecision)
                {
                    string? continueChoice = RenderWorkContinuePrompt(ctx, deltaTime);
                    if (continueChoice != null)
                    {
                        if (continueChoice == "yes")
                        {
                            // Reset for remaining work
                            ctx.ActiveWork.AwaitingContinueDecision = false;
                            ctx.ActiveWork.TotalMinutes = ctx.ActiveWork.MinutesRemainingAfterEvent;
                            ctx.ActiveWork.SimulatedMinutes = 0;
                            ctx.ActiveWork.AnimationProgress = 0f;
                            ctx.ActiveWork.AnimationDurationSeconds = Math.Clamp(0.5f + (ctx.ActiveWork.TotalMinutes * 0.02f), 0.5f, 2.0f);
                            ctx.ActiveWork.EventInterrupted = false;
                        }
                        else
                        {
                            // User declined to continue - clear work state
                            ctx.ActiveWork = null;
                        }
                    }
                    continue;
                ...
                bool workComplete = ProcessWorkTick(ctx, deltaTime);
                if (workComplete)
                {
                    // Work complete - call completion handler
                    var work = new WorkRunner(ctx);
                    work.CompleteWork();
                    continue;
                }

                // During active work, skip normal input processing
                // Just render the world with progress bar
                RenderWorkFrame(ctx, deltaTime);
                continue;
            }
    ...
    /// <summary>
    /// Process one tick of work simulation.
    /// Returns true when work is complete.
    /// </summary>
    private static bool ProcessWorkTick(GameContext ctx, float deltaTime)
    {
        var work = ctx.ActiveWork!;

        // Update animation progress using stored duration
        float animDuration = work.AnimationDurationSeconds;
        work.AnimationProgress += deltaTime / animDuration;
        work.AnimationProgress = Math.Min(work.AnimationProgress, 1f);

        // Calculate how many minutes to simulate this frame
        // Spread total minutes across the animation duration
        float minutesPerSecond = work.TotalMinutes / animDuration;
        int minutesToSimulate = (int)(minutesPerSecond * deltaTime);
        minutesToSimulate = Math.Max(1, minutesToSimulate); // At least 1 minute
        minutesToSimulate = Math.Min(minutesToSimulate, work.TotalMinutes - work.SimulatedMinutes);
        ...
        if (minutesToSimulate > 0)
        {
            ...
            for (int i = 0; i < minutesToSimulate; i++)
            {
                ctx.Update(1, work.Activity);
                work.SimulatedMinutes++;

                // Check for events or death
                if (ctx.EventOccurredLastUpdate)
                {
                    work.EventInterrupted = true;
                    work.MinutesRemainingAfterEvent = work.TotalMinutes - work.SimulatedMinutes;

                    // If event aborted the work, complete immediately
                    if (ctx.LastEventAborted)
                    {
                        return true;
                    }

                    // Otherwise, ask if player wants to continue (if there's time remaining)
                    if (work.MinutesRemainingAfterEvent > 0)
                    {
                        work.AwaitingContinueDecision = true;
                        return false; // Not complete yet - waiting for decision
                    }
                    break;
                }
                if (!ctx.player.IsAlive)
        ...
        // Work is complete when animation is done AND simulation is done (or player died/event interrupted)
        bool animDone = work.AnimationProgress >= 1f;
        bool simDone = work.SimulatedMinutes >= work.TotalMinutes;
        bool interrupted = work.EventInterrupted || !ctx.player.IsAlive;

        return animDone && (simDone || interrupted);
    }

    /// <summary>
    /// Render a frame during active work with progress bar and stats.
    /// </summary>
    private void RenderWorkFrame(GameContext ctx, float deltaTime)
    {
        var work = ctx.ActiveWork!;
        var worldRenderer = DesktopRuntime.WorldRenderer;
        ...
        // Render stats panel - this stays visible during work!
        RenderStatsPanel(ctx);

        // Render work progress bar at bottom
        RenderWorkProgressBar(work);
    ...
    /// <summary>
    /// Render the work progress bar at the bottom of the screen (green color).
    /// </summary>
    private static void RenderWorkProgressBar(GameContext.ActiveWorkState work)
    {
        float progress = (float)work.SimulatedMinutes / work.TotalMinutes;
        progress = Math.Min(progress, 1f);
        ...
        if (ImGui.Begin("##work_progress", flags))
        {
            // Green color for work progress
            ImGui.PushStyleColor(ImGuiCol.PlotHistogram, new Vector4(0.3f, 0.65f, 0.4f, 1f));
            string statusText = $"{char.ToUpper(work.ActivityName[0])}{work.ActivityName.Substring(1)}...";
            ImGui.ProgressBar(progress, new Vector2(-1, barHeight), $"{statusText} ({work.SimulatedMinutes}/{work.TotalMinutes} min)");
            ImGui.PopStyleColor();
        }
        ImGui.End();
    }
    ...
    /// <summary>
    /// Render the continue work prompt after an event interruption.
    /// Returns "yes", "no", or null if still waiting.
    /// </summary>
    private static string? RenderWorkContinuePrompt(GameContext ctx, float deltaTime)
    {
        var work = ctx.ActiveWork!;
        var worldRenderer = DesktopRuntime.WorldRenderer;
        string? result = null;
        ...
        ImGui.Begin("Continue?", ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoCollapse);

        string prompt = $"Continue {work.ActivityName}? ({work.MinutesRemainingAfterEvent} min remaining)";
