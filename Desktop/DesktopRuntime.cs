...
public static class DesktopRuntime
{
    ...
    public static IconRenderer? IconRenderer { get; set; }
    ...
    public static void Initialize(WorldRenderer worldRenderer, OverlayManager overlays, ActionPanel actionPanel,
        InputHandler inputHandler, TilePopup tilePopup, IconRenderer iconRenderer)
    {
    ...
    private static void RenderMinimalStatePanel(GameContext ctx)
    {
        ...
        // Derived from the activity in progress - there is no separate status to keep in sync.
        ImGui.Text(ActivityConfig.Get(ctx.CurrentActivity).StatusText);
